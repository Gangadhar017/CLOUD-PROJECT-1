...
export const queueSubmission = async (job: SubmissionJob): Promise<void> => {
  try {
...
export const dequeueSubmission = async (): Promise<SubmissionJob | null> => {
  try {
...
export const markSubmissionFailed = async (submissionId: string, error: string): Promise<void> => {
  try {
    await redis.hdel(PROCESSING_KEY, submissionId);
...
export const requeueDeadLetter = async (): Promise<number> => {
  try {
    const jobs: string[] = [];
    let job;
