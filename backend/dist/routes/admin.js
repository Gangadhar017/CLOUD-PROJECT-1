...
router.get('/stats', async (req, res, next) => {
    try {
        const [totalUsers, totalContests, totalSubmissions, activeContests, pendingSubmissions,] = await Promise.all([
            prisma.user.count(),
...
router.post('/contests/:id/finalize', async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
